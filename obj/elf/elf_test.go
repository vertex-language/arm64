...
// This is an external round trip, not a self-check: it writes a real ELF
// object and hands it to objdump, a tool this tree did not write, so a
// mistake shared between the writer and a test asserting on the writer's own
// idea of "correct" has nowhere to hide. It is skipped rather than failed
// when objdump is not on PATH, because a missing reference tool is an
...
func TestWriteReadWithObjdump(t *testing.T) {
	...
	dir := t.TempDir()
	path := filepath.Join(dir, "hello.o")
	f, err := os.Create(path)
	if err != nil {
		t.Fatal(err)
	}
	if err := elfw.Write(f, o); err != nil {
		f.Close()
	...
	for _, want := range []string{
		"R_AARCH64_ADR_PREL_PG_HI21 msg",
		"R_AARCH64_ADD_ABS_LO12_NC msg",
		"R_AARCH64_CALL26",
		"puts",
	} {
	...
	for _, want := range []string{"main", "msg", "*UND*"} {
