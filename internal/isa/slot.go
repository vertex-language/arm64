...
const (
	...
	// ImmShiftLeft is LSL (immediate)'s shift-by-n rewritten as UBFM's
	// immr:imms pair: immr = (width-n) mod width, imms = width-1-n. The slot
	// this is stated on is the field immr lands in; imms is a sibling this
	// rule computes and encode/ places into Imms.
	ImmShiftLeft

	// ImmShiftRight is LSR and ASR (immediate)'s: immr = n, imms = width-1.
	// The two share this rule because UBFM and SBFM only differ in which
