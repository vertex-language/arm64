...
func encodeImm(f *isa.Form, i int, s isa.Slot, v val, explicitShift bool, prev int64) (immResult, error) {
	...
	case isa.ImmShiftLeft:
		w := uint64(FormWidth(f))
		if v.imm < 0 || uint64(v.imm) >= w {
			return immResult{}, &RangeError{f, i, v.imm,
				"a shift from 0 to " + strconv.Itoa(int(w)-1)}
		}
		n := uint64(v.imm)
		return immResult{
			value:   (w - n) % w,
			sibling: w - 1 - n, hasSibling: true, siblingClass: isa.ClassImm,
		}, nil

	case isa.ImmShiftRight:
		w := uint64(FormWidth(f))
		if v.imm < 0 || uint64(v.imm) >= w {
			return immResult{}, &RangeError{f, i, v.imm,
				"a shift from 0 to " + strconv.Itoa(int(w)-1)}
