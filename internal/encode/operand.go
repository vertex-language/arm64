...
func regNum(c isa.Class, r reg.Reg) (uint64, string) {
	...
	// Every remaining register class is a plain number in its own file. isa.Class
	// and reg.Class are two different enumerations of the same idea — one keyed
	// to a form's slot, one keyed to a register's own type — so a slot's class
	// is translated to its reg.Class rather than compared to it directly.
	want, ok := regClassOf[c]
	if !ok || c.File() != r.Class().File() || c.File() == reg.FileNone {
...
// regClassOf maps an isa.Class naming a plain-number register slot to the
// reg.Class its argument must satisfy. The classes isa handles with their own
// switch cases above — ClassX, ClassW, ClassXsp, ClassWsp, ClassPg, ClassSys —
// are absent here on purpose: they accept a reading of register 31 that no
// single reg.Class states, which is exactly why they needed a case of their
// own instead of a table entry.
var regClassOf = map[isa.Class]reg.Class{
	isa.ClassV:     reg.ClassV,
	isa.ClassQ:     reg.ClassQ,
	isa.ClassD:     reg.ClassD,
	isa.ClassS:     reg.ClassS,
	isa.ClassH:     reg.ClassH,
	isa.ClassB:     reg.ClassB,
	isa.ClassVArr:  reg.ClassVArr,
	isa.ClassVLane: reg.ClassVLane,
	isa.ClassZ:     reg.ClassZ,
	isa.ClassP:     reg.ClassP,
}
