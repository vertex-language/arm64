...
- **No atomics.** `LDXR`/`STXR` and their acquire-release variants, the LSE single-instruction family (`LDADD`, `SWP`, `CAS` and the rest), and `LDAR`/`STLR` are all absent. `DMB`/`DSB`/`ISB` are present, so a barrier can be emitted but nothing it would order can be.
