...
- **The table is the base integer, branch, and load/store instruction set: 134 forms over roughly seventy mnemonics.** `table_base.go` declares the ALU block (immediate, shifted-register, extended-register), the logical and bitfield families, move-wide and `ADR`/`ADRP`, the shift/multiply/divide group, conditional select, the unary data-processing group, the full branch family, scalar load/store including pairs and literals, the barrier and exception instructions, and `MRS`/`MSR` for the common system registers. `reg/sve.go` and `reg/vec.go` declare the `Z`, `P`, and vector-with-arrangement register types, and nothing in the table reaches any of them yet — SVE, SME, and Advanced SIMD are each a tranche of their own, the same way AVX and AVX-512 are named-but-unbuilt on the amd64 side of this tree.
...
- **No SVE, SME, or Advanced SIMD.** See above. The vector encoding families — the entire `V`/`Q`/`D`/`S`/`H`/`B` scalar-view surface and every SIMD-with-arrangement form — are registers with no instructions yet.
