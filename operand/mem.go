...
// Indexed sets a register offset: [Xn, Xm, LSL #s] or [Xn, Wm, SXTW #s].
//
// It is not named Index: the field it sets already claims that identifier,
// and a method and a field sharing a name is a Go compile error, not a stutter
// to live with. The index is a reg.Reg rather than a type parameter because a
// method cannot take one. Passing anything but an X or a W is caught by
// Validate; passing a non-register does not compile.
func (m Mem) Indexed(idx reg.Reg, ext Extend, amount uint8) Mem {
	if m.bad != "" {
		return m
	}
