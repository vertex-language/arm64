...
// in the word: 0 or more zeros, then 1 or more ones, then 0 or more zeros.
//
// Adding v's lowest set bit to v clears the entire run and carries into the
// bit above it — 0b0111_1000 + 0b0000_1000 is 0b1000_0000 — and that carry
// lands somewhere the original run never touched only when the run was
// contiguous to begin with. A value with a second, separated run of ones
// still has some of those bits set after the addition, which is what the
// AND catches.
func isShiftedMask(v uint64) bool {
	if v == 0 {
		return false
	}
	lsb := v & -v
